...
def fetch(url):
    '''页面下载'''
    if rds.sadd('REQUESTED_URL', url):  # 添加访问记录
...
class Spider(Thread):
    ...
    def run(self):
        while True:
            ...
            html = fetch(url)            # 抓取
            url_list = set(url.encode('utf-8') for url in parse(html))  # 解析
            url_list -= rds.smembers('REQUESTED_URL')  # 去重
...
def main(max_threads):
    # 添加初始任务
    print('Start')
    rds.lpush('TODO_LIST', 'http://m.sohu.com/')  # 设置初始 URL
    ...
    # 循环检查是否已经完成全部工作
    while True:
        if rds.llen('TODO_LIST') == 0 and is_all_spider_in_idle(spiders):
            # 仅当待抓取列表为空且所有爬虫为空闲状态时，程序退出
            print('All works done, exit')
            sys.exit(0)
        else:
            print('Requested %s' % rds.scard('REQUESTED_URL'))
            time.sleep(1)
